...
window.onload = function() {
	...
	if (body_element.classList.contains('isfooter')) {
		document.getElementById('adkviz_remark_02').classList.remove('d-none');
	} else {
		document.getElementById('adkviz_remark_01').classList.remove('d-none');
	}
...
for (var i = 0; i < messendger_mass.length; i++) {
	messendger_mass[i].addEventListener('click', function() {
		choiseMessenger(this)
	});
}
// var back_to_choise = document.querySelector('.back_to_choise');

// back_to_choise.onclick = function() {
// 	choisePhone();
// }

function choiseMessenger(elem){
	elem.classList.add('choised');
	document.getElementById('messenger_choise').value = elem.id;
}

function choisePhone(){
	for (var i = 0; i < messendger_mass.length; i++) {
		messendger_mass[i].classList.remove('choised');
	}
	document.getElementById('messenger_choise').value = "Не выбран";
} 

//сетит номер мессенджера в поле телефона 
function isMessengerChoise(){
  
  for (var i = 0; i < messendger_mass.length; i++) {
  	if (messendger_mass[i].classList.contains('choised')){
  	  document.getElementById('phone_input').type = 'text';
  		document.getElementById('phone_input').value = document.getElementById('messenger_input').value;
  	}
  }
...
$(document).ready(function() {
   $(".final_form").submit(function() {
   	if(setPoliticActive() && setDopMessage() && isMessengerChoise()){
			$.ajax({
         type: "POST",
         url: "simple_mail.php",
         data: $(this).serialize()
				...
				$(this).find("input").val("");
				setOverlay();
				// location.replace(window.location.origin + "/" + document.querySelector('.alias_thank').innerText +".html");
				if(location.href.slice(-1) === "/"){
				  location.replace(location.href + "thank-you");
				} else{
				  location.replace(location.href + "/thank-you");
				}
...
function set_utm_in_value () {
	let utm_mass = location.search.split('&')
	utm_mass = utm_mass.join('<br>')
	document.getElementById('hidden_utm').value = utm_mass;
}

//алидация телефона на 12 символов
